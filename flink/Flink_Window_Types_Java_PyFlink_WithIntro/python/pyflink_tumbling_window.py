...
# PyFlink Tumbling Window (DataStream)
...
def main():
    ...
    # Use Java window assigners via gateway
    jvm = env._j_stream_execution_environment.getJavaEnv().getClass().getClassLoader()
    from pyflink.java_gateway import get_gateway
    gw = get_gateway()
    Time = gw.jvm.org.apache.flink.streaming.api.windowing.time.Time
    TumblingEventTimeWindows = gw.jvm.org.apache.flink.streaming.api.windowing.assigners.TumblingEventTimeWindows

    (ds.key_by(lambda e: e[0])
       .window(TumblingEventTimeWindows.of(Time.minutes(1)))
       .reduce(lambda a,b: (a[0], a[1]+b[1], max(a[2], b[2])))
       .map(lambda e: f"TUMBLING(user={e[0]}, total={e[1]})", output_type=Types.STRING())
       .print())

    env.execute("pyflink_tumbling_window")
